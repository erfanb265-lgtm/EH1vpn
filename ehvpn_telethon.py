...
# Telegram user session used only for interacting with Provider bots and their real buttons.
TG_API_ID = int(os.getenv("TG_API_ID", "0") or "0")
TG_API_HASH = os.getenv("TG_API_HASH")
TG_SESSION = os.getenv("TG_SESSION")
TG_PROVIDER_ENABLED = bool(TG_API_ID and TG_API_HASH and TG_SESSION)
tg = TelegramClient(StringSession(TG_SESSION), TG_API_ID, TG_API_HASH) if TG_PROVIDER_ENABLED else None
...
async def send_provider_text(bot: Bot, provider: str, text: str):
    """Legacy Business-account sender. Kept for manual admin testing."""
...
# Exact labels seen on the Provider keyboards. We click the real Telegram
# button with the user session instead of trying to fake a Business message.
PROVIDER_BUTTONS = {
    "HVPN": {
        "buy": "خرید سرویس جدید 🚀",
        "custom": "🔶 سرویس حجم دلخواه 🔶",
        "pay": "💳 پرداخت ریالی",
    },
}
...
async def _find_and_click(entity, target: str, timeout: float = 20.0):
    """Find the newest Provider message containing the exact button and click it."""
    deadline = asyncio.get_running_loop().time() + timeout
    seen = set()

    while asyncio.get_running_loop().time() < deadline:
        messages = await tg.get_messages(entity, limit=12)
        for msg in messages:
            if msg.id in seen:
                continue
            seen.add(msg.id)
            for row in (msg.buttons or []):
                for button in row:
                    try:
                        if (button.text or "").strip() == target.strip():
                            await button.click()
                            return msg
                    except Exception:
                        continue
    ...
    # Give a useful error including the buttons Telegram actually exposed.
    buttons = []
    messages = await tg.get_messages(entity, limit=8)
    for msg in messages:
        buttons.extend(_button_texts(msg))
    raise RuntimeError(
        f"Provider button not found: {target!r}. "
        f"Visible buttons: {buttons[-20:]}"
    )


async def _wait_provider_reply(entity, after_id: int | None = None, timeout: float = 20.0):
    """Wait for a new incoming Provider message."""
    deadline = asyncio.get_running_loop().time() + timeout
    while asyncio.get_running_loop().time() < deadline:
        messages = await tg.get_messages(entity, limit=8)
        for msg in messages:
            if after_id is None or msg.id > after_id:
                if not msg.out:
                    return msg
        await asyncio.sleep(0.7)
    raise RuntimeError("Provider did not reply within the timeout.")


async def _click_then_wait(entity, target: str):
    clicked = await _find_and_click(entity, target)
    return await _wait_provider_reply(entity, after_id=clicked.id)


async def _send_text_then_wait(entity, text: str):
    sent = await tg.send_message(entity, text)
    return await _wait_provider_reply(entity, after_id=sent.id)


async def provider_purchase_hvpn(order):
    """HVPN flow using the actual Provider keyboard through Telethon."""
    entity = await tg.get_entity(PROVIDER_CHAT_IDS["HVPN"])
    labels = PROVIDER_BUTTONS["HVPN"]

    # Main menu -> خرید سرویس جدید
    await _click_then_wait(entity, labels["buy"])

    # Product menu -> حجم دلخواه
    await _click_then_wait(entity, labels["custom"])

    # The provider asks for GB as plain input; send it from the user session.
    gb = int(order["quantity_gb"])
    invoice = await _send_text_then_wait(entity, str(gb))

    if PROVIDER_DEBUG:
        return invoice

    # At this point the screenshot shows the real button "پرداخت ریالی".
    # Do NOT send "پرداخت از اعتبار" as text; that was the source of the
    # previous "دستور یافت نشد" errors.
    return await _click_then_wait(entity, labels["pay"])


def provider_steps(order):
    """Fallback text flow for providers not yet migrated to button mode."""
    p = PRODUCTS[order["product_key"]]
    gb = order["quantity_gb"]

    if order["product_key"] == "mx_limitless":
        return ["خرید سرویس جدید 🚀", "Limitless", "30", "1"]
    if order["product_key"] == "mx_tunnel":
        return ["خرید سرویس جدید 🚀", "Premium", str(gb)]
    if order["product_key"] == "svn_direct_1":
        return ["اشتراک نا محدود مستقیم", "1 ماهه"]
    if order["product_key"] == "svn_direct_2":
        return ["اشتراک نا محدود مستقیم", "2 ماهه"]
    if order["product_key"].startswith("svn_tunnel_"):
        gb = order["quantity_gb"] or int(order["product_key"].split("_")[-1].replace("GB", ""))
        return ["اشتراک حجمی تانل شده", f"{gb}GB"]
    return []
...
async def start_provider_purchase(bot: Bot, order_id: int):
    ...
    if not order or order["status"] not in ("approved", "provider_queued"):
        return
    ...
    async with lock:
        ...
        if active:
            ...
            await bot.send_message(
                ADMIN_ID,
                f"⏳ سفارش #{order_id} در صف خرید {provider} قرار گرفت؛ سفارش #{active['id']} در حال انجام است."
            )
            return
        ...
        await bot.send_message(
            ADMIN_ID,
            f"🔄 خرید واقعی Provider برای سفارش #{order_id} شروع شد."
        )

        try:
            if not TG_PROVIDER_ENABLED or tg is None:
                raise RuntimeError(
                    "Telethon is not configured. Set TG_API_ID, TG_API_HASH and TG_SESSION in Render."
                )

            if provider == "HVPN":
                result = await provider_purchase_hvpn(order)
                body = result.text or result.message or ""
                urls = re.findall(r'https?://[^\s<>]+', body)
                if urls:
                    await finish_provider_order(
                        bot, order_id, urls[0].rstrip(").,]>"), body
                    )
                    return

                # No URL yet: keep it pending and expose the exact Provider
                # response to admin when debug is enabled.
                if PROVIDER_DEBUG:
                    await bot.send_message(
                        ADMIN_ID,
                        f"🧪 پاسخ HVPN:\n{body[:3500]}\n\n"
                        f"دکمه‌ها: {_button_texts(result)}"
                    )
                await bot.send_message(
                    ADMIN_ID,
                    f"⚠️ سفارش #{order_id}: مرحله خرید انجام شد ولی هنوز لینک سرویس از HVPN دریافت نشده."
                )
                return

            # Other providers: preserve the old text flow until their exact
            # button layouts are captured.
            for step_no, step in enumerate(provider_steps(order), 1):
                await send_provider_text(bot, provider, step)
                if PROVIDER_DEBUG:
                    await bot.send_message(
                        ADMIN_ID,
                        f"🧪 {provider} | مرحله {step_no} ارسال شد."
                    )
                await asyncio.sleep(1.8)

        except Exception as e:
            c = conn()
            c.execute("UPDATE orders SET status='provider_error' WHERE id=?", (order_id,))
            c.commit(); c.close()
            await bot.send_message(
                ADMIN_ID,
                f"❌ خرید Provider سفارش #{order_id} شکست خورد:\n"
                f"{type(e).__name__}: {e}"
            )
...
async def status_cmd(m: types.Message):
    ...
    await m.answer(
        ...
        f"Telethon Provider: {'SET' if TG_PROVIDER_ENABLED else 'NOT SET'}\n"
        f"DB: {DB_PATH}"
    )
...
async def startup(bot:Bot):
    ...
    if TG_PROVIDER_ENABLED:
        ...
        await bot.send_message(
            ADMIN_ID,
            f"🟢 Telethon Provider session وصل شد: @{me.username or 'بدون username'}"
        )
    else:
        await bot.send_message(
            ADMIN_ID,
            "⚠️ Telethon Provider session تنظیم نشده است. TG_API_ID / TG_API_HASH / TG_SESSION را در Render قرار دهید."
        )
